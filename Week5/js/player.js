...
function Player() {
    ...
    this.init = function() {
        ...
        for (i = 0; i < Priests.length; i++) {
            this.deck.push(new PriestCard(i+1, Priests[i], Priests[i])); // Heal cards currently cost the same as the health they heal
        }
		
		for (i = 0; i < Barons.length; i++) {
            this.deck.push(new BaronCard(i+1, Barons[i], Barons[i])); // Heal cards currently cost the same as the health they heal
        }
